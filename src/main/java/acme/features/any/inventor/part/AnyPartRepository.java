...
public interface AnyPartRepository extends AbstractRepository {
	...
	@Query("select p from Part p where p.id = :partId and p.invention.inventor.id = :inventorId and p.invention.draftMode = true")
	Part findPublishedPartByIdAndInventorId(int partId, int inventorId);

	@Query("select p from Part p where p.id = :partId and p.invention.draftMode = true")
	Part findPublishedPartById(int partId);

	@Query("select p from Part p where p.invention.inventor.id = :inventorId and p.invention.draftMode = true")
	...
	@Query("select p from Part p where p.invention.id = :inventionId and p.invention.inventor.id = :inventorId and p.invention.draftMode = true")
